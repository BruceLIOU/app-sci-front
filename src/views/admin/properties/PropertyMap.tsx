...
const PropertyMap: React.FC<PropertyMapProps> = ({ properties, onMarkerClick }) => {
  const geolocated = properties.filter((p) => p.latitude && p.longitude) as (typeof properties[number] & {
    latitude: number
    longitude: number
  })[]
    ...
    return (
      ...
      >
        <span className="text-medium-emphasis">
          Aucun bien géolocalisé — renseignez des adresses pour afficher la carte
        </span>
      </div>
    )
  }

  const positions: [number, number][] = geolocated.map((p) => [p.latitude, p.longitude])
