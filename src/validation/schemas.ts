...
export const leaseFormSchema = z.object({
	...
	rent_amount: z
		...
		.refine(
			...
			"Le loyer doit etre un nombre positif.",
		),
	charges_amount: z
		.string()
		.refine(
			(v) => v === "" || (!Number.isNaN(Number(v)) && Number(v) >= 0),
			"Les charges doivent etre un nombre positif.",
		),
	deposit_amount: z
		.string()
		.refine(
			(v) => v === "" || (!Number.isNaN(Number(v)) && Number(v) >= 0),
			"Le depot doit etre un nombre positif.",
		),
});
...
export const associateFormSchema = z.object({
	...
	shares: z
		...
		.refine(
			...
			"Les parts doivent etre entre 0 et 100.",
		),
});
